...
namespace rlsiteadminwidget_sessionstats;
...
class widget extends \local_rlsiteadmin\lib\widgetbase {
    ...
    public function get_html($fullscreen = false) {
        ...
        $headertext = get_string('dashboard_sessionstats_header', 'local_rlsiteadmin');
        ...
        $html .= '<div id="sessionstats-graph"><div class="loading-spinner"><i class="fa fa-cog fa-spin fa-3x fa-fw"></i>
<span class="sr-only">Loading...</span></div></div>';
        ...
        $html .= get_string('widget_info', 'rlsiteadminwidget_sessionstats');
        $html .= '</div>';
        $html .= '</div>';
        $html .= '</div>';
        return $html;
    ...
    public function get_css_dependencies($fullscreen = false) {
        return [
            new \moodle_url($this->get_path().'/../../css/c3.min.css'),
    ...
    public function get_js_dependencies_head($fullscreen = false) {
        return [
            new \moodle_url($this->get_path().'/../../js/d3.min.js'),
            new \moodle_url($this->get_path().'/../../js/c3.min.js'),
        ];
    }
    /**
     * Get an array of js files that are needed by the widget.
